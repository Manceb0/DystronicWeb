...
export default function StoreCatalog() {
    ...
    const categories = ["all", "microcontroller", "sensor", "motor", "power", "driver", "prototyping", "module", "display", "tool"];
        ...
        <div className="container mx-auto px-4 py-8 flex flex-col md:flex-row gap-8">
            ...
            <div className="w-full md:w-64 flex-shrink-0">
                <div className="glass-panel p-6 sticky top-24">
                    <div className="flex items-center gap-2 mb-6 text-gray-100 font-mono font-bold tracking-wider">
                        <Filter size={18} className="text-[#00f0ff]" /> FILTERS
                    </div>
                    ...
                    <div className="space-y-4">
                        <h4 className="text-xs font-bold text-gray-500 uppercase tracking-widest font-mono">Category</h4>
                        <div className="space-y-2">
                            {categories.map(cat => (
                                <label key={cat} className="flex items-center gap-3 cursor-pointer group">
                                    ...
                                    <span className={`text-sm tracking-wide capitalize font-mono ${filterMode === cat ? 'text-white' : 'text-gray-400 group-hover:text-gray-200'}`}>
                                        {cat}
                                    </span>
                                </label>
                            ))}
            ...
            <div className="flex-1">
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
                    <div>
                        <h1 className="text-3xl font-bold font-mono uppercase tracking-widest text-white">Component Catalog</h1>
                        <p className="text-gray-500 text-sm font-mono mt-2">Showing {filteredComponents.length} modules</p>
                    </div>
                    ...
                    {filteredComponents.map(comp => (
                        <div key={comp.id} className="glass-panel group flex flex-col relative overflow-hidden transition-all hover:border-[#00f0ff]/50">
                            ...
                            <div className="absolute top-3 left-3 flex flex-col gap-2 z-10">
                                {comp.stock > 0 && <span className="bg-[#39ff14]/20 text-[#39ff14] border border-[#39ff14]/30 text-[10px] font-bold px-2 py-0.5 rounded-sm uppercase tracking-widest shadow-[0_0_10px_rgba(57,255,20,0.2)]">In Stock</span>}
                            ...
                            <div className="p-5 flex flex-col flex-1 border-t border-white/5 bg-[#121215]/50">
                                <p className="text-[#00f0ff] font-mono text-[10px] uppercase tracking-widest mb-1">{comp.category}</p>
