...
import { translateCondition } from "@/i18n/helpers";
        ...
        <div className="container mx-auto px-4 py-8">
            <div className="glass-panel p-8 md:p-12 mb-8 relative overflow-hidden bg-gradient-to-r from-[#0b1122] to-[#050507]">
                ...
                <div className="relative z-10 md:w-2/3">
                    <h1 className="text-4xl font-bold font-mono uppercase tracking-widest text-white mb-4">{t("community.title")}</h1>
                    <p className="text-gray-400 font-mono mb-8 text-lg">{t("community.subtitle")}</p>
                    <div className="flex gap-4">
                        <Button variant="cyan"><UploadCloud size={18} className="mr-2" /> {t("community.createPost")}</Button>
                        <Button variant="outline"><HeartHandshake size={18} className="mr-2" /> {t("community.viewDonations")}</Button>
                    </div>
                </div>
            </div>
            ...
            <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
                ...
                <div className="flex items-center bg-[#121215] border border-white/10 rounded-sm px-3 py-2 w-full md:w-64 focus-within:border-[#00f0ff]/50 transition-colors">
                    ...
                    <input type="text" placeholder={t("community.search")} className="bg-transparent text-sm w-full focus:outline-none placeholder:text-gray-600 font-mono text-white" />
                </div>
            </div>
                    ...
                    <div key={post.id} className="glass-panel flex flex-col group hover:-translate-y-1 transition-transform relative">
                        <div className="aspect-[4/3] bg-[#050507] border-b border-white/5 relative flex items-center justify-center p-4">
                            <div className="absolute top-2 left-2 flex gap-1 z-10">
                                {post.type === "sell" ? (
                                    <span className="bg-[#00f0ff]/20 text-[#00f0ff] border border-[#00f0ff]/30 text-[10px] font-bold px-2 py-0.5 rounded-sm uppercase tracking-widest">{t("community.sale")}</span>
                                ) : (
                                    <span className="bg-[#39ff14]/20 text-[#39ff14] border border-[#39ff14]/30 text-[10px] font-bold px-2 py-0.5 rounded-sm uppercase tracking-widest">{t("community.donate")}</span>
                                )}
                                <span className="bg-[#121215]/80 text-white border border-white/10 text-[10px] font-bold px-2 py-0.5 rounded-sm uppercase tracking-widest">{translateCondition(locale, post.condition)}</span>
                            </div>
                        ...
                        <div className="p-5 flex flex-col flex-1">
                            ...
                            <div className="flex items-center justify-between mt-auto border-t border-white/5 pt-4">
                                <span className="text-xl font-bold font-mono text-white">
                                    {post.price === 0 ? <span className="text-[#39ff14]">{t("common.free")}</span> : `$${post.price.toFixed(2)}`}
                                </span>
                                <Button variant="outline" size="sm" className="text-xs">
                                    {post.type === "donate" ? t("community.request") : t("community.message")}
                                </Button>
                            </div>
                        </div>
                    </div>
